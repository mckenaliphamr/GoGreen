...
write.csv(me$all$eqtls, "eQTL_final.csv")
plot(me)
...
library(dplyr)
eQTL<- read.csv("eQTL_final.csv")

#Remove duplicate SNPs 
EQTL_list<- eQTL %>% distinct(chrom, snps, .keep_all = TRUE)
View(EQTL_list)

#Create a list of duplicate genes
write.csv(
  eQTL %>%
    group_by(gene) %>%
    count(no_rows=length(gene)), "group_by.csv")